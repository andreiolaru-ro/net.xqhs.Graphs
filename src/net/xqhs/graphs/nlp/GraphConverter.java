...
public class GraphConverter {
	...
	public GraphConverter(String str) throws Exception {
		if (str != null) {
			...
			// make output file
			String filename = "out//img//TESTCXTGRPH" + (str.substring(0, Math.min(str.length(), 15))).trim()
					+ "NLPAttern.txt";
...
//			System.out.println(str);
			g = (ContextGraph) parser.getNLPattern(parser, writer, document, NLGraphType.GRAPH);
			g.setUnitName("testGraph");

			parser.getEnhancedPlusPlusPlusPlus(document, writer);

			g = (ContextGraph) ContextPatternConverter.processCorefCP(NLGraphType.GRAPH, g, document);
			g = (ContextGraph) ContextPatternConverter.relabelEdgesWithAuxWords(g);
			g = (ContextGraph) ContextPatternConverter.removeDuplicates(NLGraphType.GRAPH, g);
			// Parser.displayContextPattern(g, true);
		}
	}

	public GraphConverter(List<String> strs, StanfordCoreNLP pipeline) throws Exception {
		if (strs != null) {
			Parser parser = new Parser();
			for (String str : strs) {
