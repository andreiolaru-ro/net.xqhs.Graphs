...
/**
 * When {@link ContextEdge} instances are added to a the graph, the behavior is
 * different from when using underlying classes. That is, the edge is attached
 * to the graph correctly even if the nodes adjacent to it are new instances
 * (are not the ones from the graph). Moreover, if any nodes adjacent to it are
 * not already in the graph (as found by label comparison), they will be added
 * to the graph automatically.
 * <p>
 * When a {@link ContextEdge} instance is added to, or removed from the graph,
 * it will also be added to / removed from the validity queue. WHen the validity
 * of an edge expires, it is removed.
 *
 * @author Andrei Olaru
 */
// TODO: add canPerformOperation throughout the various Graph classes to be sure
// that the operation will be performed.
public class ContextGraph extends PrincipalGraph implements TickReceiver {
	/**
	 * A context edge features, beside what is offered by {@link SimpleEdge}, a
	 * validity setting, so that it is able to expire at a given time after
	 * being added to a {@link ContextGraph}.
	 *
	 * @author Andrei Olaru
	 */
	public static class ContextEdge extends SimpleEdge {
		...
		public ContextEdge(Node fromNode, Node toNode, String edgeLabel,
				Offset edgeValidity) {
			super(fromNode, toNode, edgeLabel);
			if (edgeValidity == null)
				throw new IllegalArgumentException(
						"Edge validity must be an instantiated object.");
	...
	public NLGraphType t = NLGraphType.GRAPH;
	TimeKeeper theTime = null;
	PriorityQueue<Entry<Instant, ContextEdge>> validityQueue = new PriorityQueue<Entry<Instant, ContextEdge>>(
			1, new Comparator<Entry<Instant, ContextEdge>>() {
				@Override
				public int compare(Entry<Instant, ContextEdge> o1,
	...
	public ContextGraph setTimeKeeper(TimeKeeper time) {
		theTime = time;
	...
	protected ContextGraph performOperation(GraphComponent component,
			Operation operation, boolean externalCall) {
		...
		if (component instanceof ContextEdge)
			...
			case ADD:
				...
				if (!fromExisting.isEmpty())
					...
					super.performOperation(e.getFrom(), Operation.ADD,
							externalCall);

				if (!toExisting.isEmpty())
					e.setTo(toExisting.iterator().next());
				else
					super.performOperation(e.getTo(), Operation.ADD,
							externalCall);

				if (!fromExisting.isEmpty() && !toExisting.isEmpty())
					for (Edge existing : nodes.get(e.getFrom()).getOutEdges())
						if ((existing.getTo() == e.getTo())
								&& (existing.getLabel().equals(e.getLabel())))
							// edge is existing
							return this;

				validityQueue
						.add(new AbstractMap.SimpleEntry<Instant, ContextEdge>(
								theTime.now().offsetInstant(e.initialValidity),
								e));
				break;
			case REMOVE:
				for (Iterator<Entry<Instant, ContextEdge>> it = validityQueue
						.iterator(); it.hasNext();)
	...
	/**
	 * Overrides {@link TrackingGraph#add(GraphComponent)} to not allow nodes
	 * with the same label (as per the theory of context graphs) or generic
	 * nodes (having a label beginning with {@link NodeP#NODEP_LABEL}.
	 */
	@Override
	public ContextGraph add(GraphComponent component) {
		...
		if ((component instanceof Node)
				&& ((Node) component).getLabel().startsWith(NodeP.NODEP_LABEL))
			throw new IllegalArgumentException("Generic nodes are not allowed");
		// TODO this is not efficient -- a index of labels should be used
		if ((component instanceof Node)
				&& !getNodesNamed(((Node) component).getLabel()).isEmpty())
			throw new IllegalArgumentException(
					"Multiple nodes with the same name are not allowed");
	...
	public void tick(TimeKeeper ticker, Instant now) {
		...
		while (!validityQueue.isEmpty()
				&& validityQueue.peek().getKey().before(now))
