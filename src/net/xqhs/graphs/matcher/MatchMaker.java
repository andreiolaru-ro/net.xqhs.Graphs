...
public class MatchMaker {
	...
	public SimpleGraph copyGraph(SimpleGraph g) {
		...
		for (Edge e : g.getEdges()) {
			NLEdge newEdge = new NLEdgeG(iso.get(e.getFrom()), iso.get(e.getTo()), e.getLabel(),
					((NLEdge) e).getRole());
			newg.add(newEdge);
		}
	...
	SimpleGraph convertUnsolvedPartOfPatternToGraph(GraphPattern gp) {
		...
		for (Edge e : gp.getEdges().stream()
				.filter(ee -> ((NLEdge) ee).getRole().equals(ContextPatternConverter.determinerRole))
		...
		newg.addAll(iso.values().stream().distinct().collect(Collectors.toList()));
		// System.out.println("Node correspondences:" + iso.toString());
		for (Edge e : gp.getEdges()) {
			// if edge ain't an is
			if (!((NLEdge) e).getRole().equals(ContextPatternConverter.determinerRole))

			{
				...
				if (from != null && to != null) {
					...
					NLEdgeG eG = new NLEdgeG(from, to, e.getLabel(), ((NLEdge) e).getRole());
					newg.add(eG);
				} else {
					...
					if (from == null) {
						if (!((NLNodeP) e.getFrom()).isGeneric())
							...
							from = iso.get(gp.getEdges().stream()
									.filter(ee -> ((NLEdge) ee).getRole().equals(ContextPatternConverter.determinerRole)
											&& ee.getFrom().equals(ee.getFrom()))
									.findFirst().get().getTo());
							if (from == null)
								newFrom = new NLNodeG((NLNodeP) gp.getEdges().stream().filter(
										ee -> ((NLEdge) ee).getRole().equals(ContextPatternConverter.determinerRole)
												&& ee.getFrom().equals(ee.getFrom()))
										.findFirst().get().getTo());
					...
					if (to == null) {
						if (!((NLNodeP) e.getTo()).isGeneric())
							...
							to = iso.get(gp.getEdges().stream()
									.filter(ee -> ((NLEdge) ee).getRole().equals(ContextPatternConverter.determinerRole)
											&& ee.getFrom().equals(ee.getTo()))
									.findFirst().get().getTo());
							if (to == null)
								newTo = new NLNodeG((NLNodeP) gp.getEdges().stream().filter(
										ee -> ((NLEdge) ee).getRole().equals(ContextPatternConverter.determinerRole)
												&& ee.getFrom().equals(ee.getTo()))
										.findFirst().get().getTo());
					...
					NLEdgeG eG = new NLEdgeG(from == null ? newFrom : from, to == null ? newTo : to, e.getLabel(),
							((NLEdge) e).getRole());
					newg.add(eG);
					if (from == null)
						newg.add(newFrom);
	...
	public HashMap<NLNodeP, NLNodeG> convertPatternNodesToGraphNodes(GraphPattern gp) {
		HashMap<NLNodeP, NLNodeG> iso = new HashMap<NLNodeP, NLNodeG>();
//		System.out.println("Nodes in unsolved part of pattern: " + gp.getNodes().toString());
...
//					System.out.println("Found generic node of type " + newTypeG);
				} else {
					// copy node
	...
	public static NLNodeP findTypeOfGenericNode(GraphPattern gp, NodeP genericNode) {
		Edge detEdge = gp.getOutEdges(genericNode).stream()
				.filter(e -> ((NLEdge) e).getRole().equals(ContextPatternConverter.determinerRole)
						&& e.getFrom().equals(genericNode))
				.findFirst().get();
	...
	public SimpleGraph nowKiss(SimpleGraph gg, GraphPattern unsolvedPart) {
		...
		for (Edge ee : u.getEdges()) {
			...
			if (g.getNodesNamed(e.getFrom().getLabel()).isEmpty() && g.getNodesNamed(e.getTo().getLabel()).isEmpty()) {
			...
			// if the graph contains the missing node
				...
				NLEdgeG edge = new NLEdgeG((NLNodeG) g.getNodesNamed(e.getFrom().getLabel()).iterator().next(),
						(NLNodeG) e.getTo(), e.getLabel(), e.getRole());
//				System.out.println("Created edge " + e + "using node " + e.getFrom());
			...
			// if (u.contains(e.getFrom()) && !u.contains(e.getTo()))
				...
				NLEdgeG edge = new NLEdgeG((NLNodeG) e.getFrom(),
						(NLNodeG) g.getNodesNamed(e.getTo().getLabel()).iterator().next(), e.getLabel(), e.getRole());
				g.add(edge);// &connect to existing node to

			}
			// if both nodes are in graph & edge is in pattern
			else {
				g.add(new NLEdgeG((NLNodeG) g.getNodesNamed(e.getFrom().getLabel()).iterator().next(),
						(NLNodeG) g.getNodesNamed(e.getTo().getLabel()).iterator().next(), e.getLabel(), e.getRole()));
			}
		}
